  ...
  return (
    <>
      <div className="flex items-center gap-2.5 px-5 py-5 border-b border-stone-100">
        ...
        <span className="text-base font-semibold text-stone-800 tracking-tight">TerraTrip</span>
      </div>
...
export function Sidebar() {
  ...
  return (
    <>
      {/* Mobile top bar */}
      <header className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b border-stone-100 flex items-center justify-between px-4 z-30">
        <div className="flex items-center gap-2">
          ...
          <span className="text-sm font-semibold text-stone-800 tracking-tight">TerraTrip</span>
        </div>
        <button
      ...
      {/* Desktop sidebar */}
      <aside className="hidden md:flex fixed left-0 top-0 h-full w-64 bg-white border-r border-stone-100 flex-col z-40">
      ...
      {open && (
        <div className="md:hidden fixed inset-0 z-50 flex">
          ...
          />
          <aside className="relative w-72 max-w-[80%] h-full bg-white flex flex-col shadow-2xl animate-in slide-in-from-left duration-200">
            <button
              aria-label="Close menu"
              onClick={() => setOpen(false)}
              className="absolute right-3 top-3 p-1.5 rounded-lg text-stone-400 hover:bg-stone-100 hover:text-stone-600 transition-colors"
            >
              <X className="h-4 w-4" />
            </button>
