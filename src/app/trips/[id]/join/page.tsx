...
export default function JoinTripPage() {
  ...
  return (
    <div className="flex min-h-[60vh] items-center justify-center">
      <div className="w-full max-w-sm rounded-3xl border border-stone-100 bg-white p-8 text-center shadow-sm">
        {status === "joining" && (
          <>
            <div className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-2 border-amber-600 border-t-transparent" />
            <h1 className="text-base font-semibold text-stone-800">Joining trip…</h1>
        ...
        {status === "error" && (
          <>
            ...
            <h1 className="text-base font-semibold text-stone-800">Couldn&apos;t join</h1>
            <p className="mt-1 text-sm text-stone-500">{message}</p>
            <Link
              href="/trips"
              className="mt-4 inline-block rounded-xl bg-stone-100 px-4 py-2 text-sm font-medium text-stone-600 hover:bg-stone-200"
            >
              Go to trips
            </Link>
          </>
